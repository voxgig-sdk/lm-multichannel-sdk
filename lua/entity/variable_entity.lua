...
local VariableEntity = {}
VariableEntity.__index = VariableEntity


function VariableEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VariableEntity)
  self._name = "variable"
  self._client = client
...
function VariableEntity:get_name()
  return self._name
end


function VariableEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VariableEntity.new(self._client, opts)
end
...
function VariableEntity:mark_deleted()
  self._deleted = true
end


function VariableEntity:deleted()
  return true == self._deleted
end


function VariableEntity:data_set(args)
  if args ~= nil then
...
function VariableEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VariableEntity:match_set(args)
  if args ~= nil then
...
function VariableEntity:match_get()
...
function VariableEntity:stream(action, args, callopts)
...
---@param reqmatch VariableListMatch
---@param ctrl? table
---@return Variable[]
---@return string? err
function VariableEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata VariableCreateData
---@param ctrl? table
---@return Variable
---@return string? err
function VariableEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata VariableUpdateData
---@param ctrl? table
---@return Variable
---@return string? err
function VariableEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function VariableEntity:_run_op(ctx, post_done)
  local utility = self._utility
