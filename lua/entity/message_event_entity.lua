...
local MessageEventEntity = {}
MessageEventEntity.__index = MessageEventEntity


function MessageEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MessageEventEntity)
  self._name = "message_event"
  self._client = client
...
function MessageEventEntity:get_name()
  return self._name
end


function MessageEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MessageEventEntity.new(self._client, opts)
end
...
function MessageEventEntity:mark_deleted()
  self._deleted = true
end


function MessageEventEntity:deleted()
  return true == self._deleted
end


function MessageEventEntity:data_set(args)
  if args ~= nil then
...
function MessageEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MessageEventEntity:match_set(args)
  if args ~= nil then
...
function MessageEventEntity:match_get()
...
function MessageEventEntity:stream(action, args, callopts)
...
---@param reqmatch MessageEventListMatch
---@param ctrl? table
---@return MessageEvent[]
---@return string? err
function MessageEventEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function MessageEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
