...
local TrafficEntity = {}
TrafficEntity.__index = TrafficEntity


function TrafficEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TrafficEntity)
  self._name = "traffic"
  self._client = client
...
function TrafficEntity:get_name()
  return self._name
end


function TrafficEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TrafficEntity.new(self._client, opts)
end
...
function TrafficEntity:mark_deleted()
  self._deleted = true
end


function TrafficEntity:deleted()
  return true == self._deleted
end


function TrafficEntity:data_set(args)
  if args ~= nil then
...
function TrafficEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TrafficEntity:match_set(args)
  if args ~= nil then
...
function TrafficEntity:match_get()
...
function TrafficEntity:stream(action, args, callopts)
...
---@param reqmatch TrafficRemoveMatch
---@param ctrl? table
---@return Traffic
---@return string? err
function TrafficEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function TrafficEntity:_run_op(ctx, post_done)
  local utility = self._utility
