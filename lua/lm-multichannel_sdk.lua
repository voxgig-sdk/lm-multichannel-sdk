...
LmMultichannelSDK._make_feature = _make_feature


function LmMultichannelSDK.new(options)
  local self = setmetatable({}, LmMultichannelSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function LmMultichannelSDK:options_map()
...
function LmMultichannelSDK:get_utility()
  return Utility.copy(self._utility)
end


function LmMultichannelSDK:get_root_ctx()
  return self._rootctx
end


function LmMultichannelSDK:prepare(fetchargs)
...
function LmMultichannelSDK:direct(fetchargs)
...
-- by passing it.
function LmMultichannelSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function LmMultichannelSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "LmMultichannelSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function LmMultichannelSDK:Content(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Message(data)
...
-- Idiomatic facade: client:MessageEvent():list() / client:MessageEvent():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:MessageEvent(data)
  local EntityMod = require("entity.message_event_entity")
  if data == nil then
    if self._message_event == nil then
      self._message_event = EntityMod.new(self, nil)
    end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Option(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Schedule(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Self(data)
...
-- Idiomatic facade: client:SelfAdmin():list() / client:SelfAdmin():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:SelfAdmin(data)
  local EntityMod = require("entity.self_admin_entity")
  if data == nil then
    if self._self_admin == nil then
      self._self_admin = EntityMod.new(self, nil)
    end
    return self._self_admin
  end
  return EntityMod.new(self, data)
end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Template(data)
...
-- Idiomatic facade: client:Traffic():list() / client:Traffic():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Traffic(data)
  local EntityMod = require("entity.traffic_entity")
  if data == nil then
    if self._traffic == nil then
      self._traffic = EntityMod.new(self, nil)
    end
...
-- Idiomatic facade: client:TrafficFile():list() / client:TrafficFile():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:TrafficFile(data)
  local EntityMod = require("entity.traffic_file_entity")
  if data == nil then
    if self._traffic_file == nil then
      self._traffic_file = EntityMod.new(self, nil)
    end
    return self._traffic_file
  end
  return EntityMod.new(self, data)
end


-- Idiomatic facade: client:Variable():list() / client:Variable():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LmMultichannelSDK:Variable(data)
  local EntityMod = require("entity.variable_entity")
  if data == nil then
    if self._variable == nil then
      self._variable = EntityMod.new(self, nil)
    end
...
function LmMultichannelSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = LmMultichannelSDK.new(sdkopts)
  sdk.mode = "test"
