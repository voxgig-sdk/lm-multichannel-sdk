...
[MCP](https://modelcontextprotocol.io) server exposing the LmMultichannel SDK as
two agent tools — `lm-multichannel_list` and `lm-multichannel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-multichannel-mcp)
make build

# 2. Provide credentials via the environment
export LM_MULTICHANNEL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user lm-multichannel \
  -- /absolute/path/to/lm-multichannel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./lm-multichannel-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// lm-multichannel_list: first page of records
{ "entity": "message_event" }
{ "entity": "message_event", "query": { } }

// lm-multichannel_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-multichannel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user lm-multichannel \
     -- "$PWD"/dist/*/lm-multichannel-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `lm-multichannel_list` and `lm-multichannel_load` tools now appear
   in new sessions. Ask the agent to *"list message_event using lm-multichannel"*
   and it calls `lm-multichannel_list` with `{"entity":"message_event"}`.
...
```sh
export LM_MULTICHANNEL_APIKEY=sk_live_xxx            # API key
export LM_MULTICHANNEL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `lm-multichannel_list` tool
...
### Call the `lm-multichannel_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `lm-multichannel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `lm-multichannel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LM_MULTICHANNEL_APIKEY` | API key sent with every request. |
| `LM_MULTICHANNEL_BASE` | Optional override of the API base URL. |
...
```sh
./lm-multichannel-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"lm-multichannel_load","arguments":{"entity":"content","query":{"id":1}}}}'
```
