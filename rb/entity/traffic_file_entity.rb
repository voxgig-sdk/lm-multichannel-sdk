...
class TrafficFileEntity
  ...
  def make
    opts = @_entopts.dup
    TrafficFileEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LmMultichannelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TrafficFile, Hash] the current TrafficFile data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LmMultichannelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TrafficFile fields)
  def match_get
  ...
  #
  # @param reqmatch [TrafficFileLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.TrafficFile.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [TrafficFile, Hash] the loaded TrafficFile; raises LmMultichannelError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LmMultichannelHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [TrafficFileListMatch, Hash, nil] match filter (any subset of
  #   TrafficFile fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TrafficFile>, Array] the matching TrafficFile items; raises LmMultichannelError on failure
