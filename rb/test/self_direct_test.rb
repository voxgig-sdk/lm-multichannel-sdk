...
class SelfDirectTest < Minitest::Test
  def test_direct_load_self
    setup = self_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-self", setup[:live] ? "live" : "unit")
    if _should_skip
