...
class MessageEventDirectTest < Minitest::Test
  def test_direct_list_message_event
    setup = message_event_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-message_event", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["message_event01"].each do |_live_key|
...
def message_event_direct_setup(mockres)
