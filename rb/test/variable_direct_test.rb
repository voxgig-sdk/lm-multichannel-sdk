...
class VariableDirectTest < Minitest::Test
  def test_direct_list_variable
    setup = variable_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-variable", setup[:live] ? "live" : "unit")
    if _should_skip
...
def variable_direct_setup(mockres)
