...
class VariableEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return VariableEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Variable]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Variable().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: VariableCreateData, ctrl=None) -> Variable:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: VariableUpdateData, ctrl=None) -> Variable:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
