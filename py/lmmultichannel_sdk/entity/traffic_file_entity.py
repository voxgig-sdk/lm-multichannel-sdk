...
from lmmultichannel_sdk.lmmultichannel_types import (
    TrafficFile,
    TrafficFileLoadMatch,
    TrafficFileListMatch,
)
...
class TrafficFileEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TrafficFileEntity(self._client, opts)
    ...
    def match_get(self) -> TrafficFile:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TrafficFile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TrafficFile().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[TrafficFile]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.TrafficFile().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
