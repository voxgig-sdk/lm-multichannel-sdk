# LmMultichannel SDK test runner
...
class LmMultichannelTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LmMultichannelTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LmMultichannelTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LmMultichannelTestRunner.getenv("LM_MULTICHANNEL_TEST_LIVE")
        override = LmMultichannelTestRunner.getenv("LM_MULTICHANNEL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LmMultichannelTestRunner.getenv(key)
        ...
        explain = LmMultichannelTestRunner.getenv("LM_MULTICHANNEL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["LM_MULTICHANNEL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if LmMultichannelTestRunner._test_control is not None:
            return LmMultichannelTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LmMultichannelTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LmMultichannelTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LmMultichannelTestRunner.load_test_control()
...
def load_env_local():
    LmMultichannelTestRunner.load_env_local()


def env_override(m):
    return LmMultichannelTestRunner.env_override(m)


def entity_data(v):
    return LmMultichannelTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LmMultichannelTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LmMultichannelTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LmMultichannelTestRunner.load_test_control()


def live_client_options():
    return LmMultichannelTestRunner.live_client_options()


def live_delay_ms():
    return LmMultichannelTestRunner.live_delay_ms()
