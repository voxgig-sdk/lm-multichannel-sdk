...
class TestMessageEventEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.MessageEvent(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.MessageEvent(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LMMULTICHANNEL_TEST_MESSAGE_EVENT_ENTID JSON to run live")
        client = setup["client"]
        ...
        # LIST
        message_event_ref01_ent = client.MessageEvent(None)
        message_event_ref01_match = {
            "message_id": setup["idmap"]["message01"],
        }

        message_event_ref01_list_result = message_event_ref01_ent.list(message_event_ref01_match, None)
        assert isinstance(message_event_ref01_list_result, list)
...
def _message_event_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LMMULTICHANNEL_TEST_MESSAGE_EVENT_ENTID"))
