...
class TestTrafficFileDirect:

    def test_should_direct_list_traffic_file(self):
        setup = _traffic_file_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-traffic_file", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_traffic_file(self):
        setup = _traffic_file_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-traffic_file", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _traffic_file_direct_setup(mockres):
