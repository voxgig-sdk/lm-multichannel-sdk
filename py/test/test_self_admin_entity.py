...
class TestSelfAdminEntity:

    def test_should_create_instance(self):
        testsdk = LmMultichannelSDK.test(None, None)
        ent = testsdk.SelfAdmin(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _self_admin_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "self_admin." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LM_MULTICHANNEL_TEST_SELF_ADMIN_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        self_admin_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.self_admin")))
        self_admin_ref01_data = None
        if len(self_admin_ref01_data_raw) > 0:
            self_admin_ref01_data = helpers.to_map(self_admin_ref01_data_raw[0][1])

        # UPDATE
        self_admin_ref01_ent = client.SelfAdmin(None)
        self_admin_ref01_data_up0_up = {
        }

        self_admin_ref01_resdata_up0 = helpers.to_map(runner.entity_data(self_admin_ref01_ent.update(self_admin_ref01_data_up0_up, None)))
        assert self_admin_ref01_resdata_up0 is not None



def _self_admin_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/self_admin/SelfAdminTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "LM_MULTICHANNEL_TEST_SELF_ADMIN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LM_MULTICHANNEL_TEST_SELF_ADMIN_ENTID"))
