...
// LmMultichannel SDK retry feature
...
class LmMultichannelRetryFeature extends LmMultichannelBaseFeature
{
    ...
    public function init(LmMultichannelContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LmMultichannelContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LmMultichannelContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LmMultichannelContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
