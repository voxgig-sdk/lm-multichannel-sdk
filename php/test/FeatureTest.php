...
// LmMultichannel SDK feature test
//
...
class FtCtrl extends LmMultichannelControl
{
...
class FtClient
{
    ...
    private LmMultichannelUtility $utility;

    public function __construct(LmMultichannelUtility $utility, array $options)
    {
    ...
    public function get_utility(): LmMultichannelUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LmMultichannelContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LmMultichannelContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LmMultichannelContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LmMultichannelConfig::make_feature($name);
    ...
    private static function buildUrl(LmMultichannelSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LmMultichannelError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LmMultichannelResponse($fetched);
        }
    ...
    private function fail(LmMultichannelContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LmMultichannelContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
