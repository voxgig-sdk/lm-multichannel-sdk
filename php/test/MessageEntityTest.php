...
class MessageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LM_MULTICHANNEL_TEST_MESSAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function message_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LM_MULTICHANNEL_TEST_MESSAGE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["LM_MULTICHANNEL_TEST_MESSAGE_ENTID"]);
