...
// LmMultichannel SDK Variable entity
...
class VariableEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new VariableEntity($this->_client, $opts);
    }

    /**
     * @param Variable|array $args Variable data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LmMultichannelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Variable|array The current Variable data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Variable fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LmMultichannelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Variable fields).
     */
    ...
    /**
     * List Variable items matching the given filter.
     *
     * @param VariableListMatch|array|null $reqmatch Match filter (any subset
     *   of Variable fields) as an assoc-array; VariableListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Variable[]|array A list of Variable items as assoc-arrays at
     *   the SDK boundary; throws LmMultichannelError on failure (item-5 convention).
     */
     ...
     *
     * @param VariableCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed VariableCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Variable|array The created Variable as an assoc-array at the
     *   SDK boundary; throws LmMultichannelError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = LmMultichannelHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param VariableUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed VariableUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Variable|array The updated Variable as an assoc-array at the
     *   SDK boundary; throws LmMultichannelError on failure (item-5 convention).
     */
