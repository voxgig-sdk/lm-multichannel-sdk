...
boru-driven command-line client **and** interactive REPL for the LmMultichannel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-multichannel-cli)
make build

# 2. See usage (words, entities, env vars)
./lm-multichannel-cli --help
...
./lm-multichannel-cli
lm-multichannel> load 1 content
lm-multichannel> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-multichannel-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/lm-multichannel-cli load 1 content
   ```
...
```sh
./lm-multichannel-cli load 1 content          # scalar shorthand for {id:1}
./lm-multichannel-cli load '{id:1}' content     # explicit match map
```
...
./lm-multichannel-cli load 1 content
```
...
Run with no arguments to open a REPL (prompt `lm-multichannel>`). Each line is
evaluated as its own boru expression:

```text
$ ./lm-multichannel-cli
lm-multichannel> load 1 content
lm-multichannel> /help
lm-multichannel> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/lm-multichannel-cli`. |
...
(`./lm-multichannel-cli <expr>`) and interactively (the REPL), and expressions compose the
