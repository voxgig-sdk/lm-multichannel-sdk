...
module LmMultichannelUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LmMultichannelResponse.new({})
    ...
    if fetch_err
      ...
      response = LmMultichannelResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LmMultichannelResponse.new(fetched)
    else
