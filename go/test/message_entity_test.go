...
func TestMessageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMMULTICHANNEL_TEST_MESSAGE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMMULTICHANNEL_TEST_MESSAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMMULTICHANNEL_TEST_MESSAGE_ENTID"])
