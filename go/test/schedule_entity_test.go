...
func TestScheduleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMMULTICHANNEL_TEST_SCHEDULE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func scheduleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMMULTICHANNEL_TEST_SCHEDULE_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"LMMULTICHANNEL_TEST_SCHEDULE_ENTID": idmap,
		"LMMULTICHANNEL_TEST_LIVE":      "FALSE",
		"LMMULTICHANNEL_TEST_EXPLAIN":   "FALSE",
		"LMMULTICHANNEL_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["LMMULTICHANNEL_TEST_SCHEDULE_ENTID"])
