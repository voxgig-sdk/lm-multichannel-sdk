...
import (
	...
	omni "github.com/voxgig-sdk/lm-multichannel-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LmMultichannelError); is {
				out := map[string]any{
					"name":    "LmMultichannelError",
...
func omniCtx(arg any, client *sdk.LmMultichannelSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LmMultichannelSDK); is {
			client = live
