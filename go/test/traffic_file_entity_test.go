...
func TestTrafficFileEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.TrafficFile(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.TrafficFile(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMMULTICHANNEL_TEST_TRAFFIC_FILE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		trafficFileRef01Match := map[string]any{}

		trafficFileRef01ListResult, err := trafficFileRef01Ent.List(trafficFileRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, trafficFileRef01ListOk := trafficFileRef01ListResult.([]any)
		if !trafficFileRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", trafficFileRef01ListResult)
		}
...
func traffic_fileBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMMULTICHANNEL_TEST_TRAFFIC_FILE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMMULTICHANNEL_TEST_TRAFFIC_FILE_ENTID"])
