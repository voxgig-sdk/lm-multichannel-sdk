...
func TestMessageEventEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		...
		if ent == nil {
			t.Fatal("expected non-nil MessageEventEntity")
		}
	})
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMMULTICHANNEL_TEST_MESSAGE_EVENT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		messageEventRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.message_event", setup.data)))
		var messageEventRef01Data map[string]any
		if len(messageEventRef01DataRaw) > 0 {
			messageEventRef01Data = core.ToMapAny(messageEventRef01DataRaw[0][1])
		}
...
func message_eventBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "message_event", "MessageEventTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read message_event test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse message_event test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("LMMULTICHANNEL_TEST_MESSAGE_EVENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMMULTICHANNEL_TEST_MESSAGE_EVENT_ENTID"])
