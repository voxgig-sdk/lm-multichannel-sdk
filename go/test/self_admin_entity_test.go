...
func TestSelfAdminEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMMULTICHANNEL_TEST_SELF_ADMIN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		selfAdminRef01ResdataUp0 := core.ToMapAny(selfAdminRef01ResdataUp0Result)
...
func self_adminBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMMULTICHANNEL_TEST_SELF_ADMIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMMULTICHANNEL_TEST_SELF_ADMIN_ENTID"])
