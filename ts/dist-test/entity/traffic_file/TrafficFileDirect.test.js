...
(0, node_test_1.describe)('TrafficFileDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.LmMultichannelSDK({
    ...
    (0, node_test_1.test)('direct-load-traffic_file', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-traffic_file', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-traffic_file', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-traffic_file', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.LM_MULTICHANNEL_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.LmMultichannelSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.LM_MULTICHANNEL_APIKEY,
        }));
        let idmap = env['LM_MULTICHANNEL_TEST_TRAFFIC_FILE_ENTID'];
    ...
    const client = new __1.LmMultichannelSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=TrafficFileDirect.test.js.map