...
(0, node_test_1.describe)('ScheduleDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-schedule', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-schedule', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'LM_MULTICHANNEL_TEST_SCHEDULE_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['LM_MULTICHANNEL_TEST_SCHEDULE_ENTID'];
