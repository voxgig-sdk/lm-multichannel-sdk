...
(0, node_test_1.describe)('TemplateDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-template', async (t) => {
        ...
        const result = await client.direct({
            path: 'templates/{id}/reviews/{channel_id}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-template', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-template', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LM_MULTICHANNEL_TEST_TEMPLATE_ENTID'];
