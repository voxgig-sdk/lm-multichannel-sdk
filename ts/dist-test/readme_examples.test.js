...
const __1 = require("..");
const SDK_NAME = 'LmMultichannelSDK';
...
function isRunnable(code) {
    return (/new\s+LmMultichannelSDK\b/.test(code) ||
        /\bLmMultichannelSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LmMultichannelSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LmMultichannelSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
