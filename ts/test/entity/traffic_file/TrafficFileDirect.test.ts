...
describe('TrafficFileDirect', async () => {
  ...
  test('direct-load-traffic_file', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-traffic_file', setup.live)) return
  ...
  test('direct-list-traffic_file', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-traffic_file', setup.live)) return
    const { client, calls } = setup
