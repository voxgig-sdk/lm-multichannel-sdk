...
describe('VariableDirect', async () => {
  ...
  test('direct-list-variable', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-variable', setup.live)) return
    if (skipIfMissingIds(t, setup, ["template01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LM_MULTICHANNEL_TEST_VARIABLE_ENTID']
