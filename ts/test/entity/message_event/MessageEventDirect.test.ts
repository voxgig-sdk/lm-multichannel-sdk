...
describe('MessageEventDirect', async () => {
  ...
  test('direct-list-message_event', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-message_event', setup.live)) return
    if (skipIfMissingIds(t, setup, ["message_event01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LM_MULTICHANNEL_TEST_MESSAGE_EVENT_ENTID']
